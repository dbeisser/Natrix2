...
# Filtere taxids, die im tax_df Index vorhanden sind
existing_taxids = [taxid for taxid in taxids if taxid in tax_df.index]
tax_df = tax_df.loc[existing_taxids, :]
...
for i in querynames:
    ...
    # Filtere dbseqid, die im tax_df vorhanden sind
    valid_dbseqids = [x for x in blast_res['dbseqid'] if x in tax_df.index]
    if not valid_dbseqids:
        continue  # keine gültigen TaxIDs, nächster Query
    
    tax_res = tax_df.loc[valid_dbseqids, :]
    
    # Setze Index von tax_res auf Index von blast_res gefiltert nach valid_dbseqids
    filtered_blast_res = blast_res[blast_res['dbseqid'].isin(valid_dbseqids)]
    tax_res = tax_res.set_index(filtered_blast_res.index)
    
    blast_res = pd.concat([filtered_blast_res, tax_res], axis=1)
