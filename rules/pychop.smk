...
if config['dataset']['nanopore']:
    ...
    rule pychopper_rescue:
        ...
        shell:
            ...
            printf ">SSP\n$forward_primer\n>VNP\n$reverse_primer" > custom_primers.fasta;
            echo "+:SSP,-VNP|-:VNP,-SSP" > config_primers.txt;
            pychopper -m edlib -x rescue -b custom_primers.fasta -Q {params.qual} -c config_primers.txt -r {output.unclass_pdf} -u {output.unclass_unclass_fastq} -w {output.unclass_rescue_fastq} {input.unclass_fastq} {output.unclass_out_fastq};
            """
